...
[~,P,~] = dlqr(sysd.A,sysd.B,Q,Re); % Terminal cost function & LQR gain
% A_cl = sysd.A+sysd.B*K_lqr;

%% ----- Lyapunov Inequality Terminal Cost & Stability Guarantee ------ %%
K = sysd.B\(eye(3)-sysd.A)-0.1*[1 1 1]; 
% K1 = [-2 -0.8  0.1];
Z = Q+K'*Re*K;
P_lyap = dlyap((sysd.A+sysd.B*K),Z); % Terminal cost from Lyapunov's equation 
[~,posdef] = chol(P_lyap);  % Checking if the P is positive-definite

if posdef==0
    P = P_lyap; % Stability guarantee
end
...
% Constrained MPC parameters and Feasible Sets
xmax = [2*pi;12;8]-x_ss;
xmin = [-2*pi;-12;-8]-x_ss;
umax = 10-u_ss;
umin = -10-u_ss;
...
subplot(2,1,1);
plot(0:k_sim,x_cloop(1,:),'Linewidth',2);
grid on
hold on
plot(0:k_sim,x_cloop(2,:),'Linewidth',2);
plot(0:k_sim,x_cloop(3,:),'Linewidth',2);
...
plot(Fset_x,'color','yellow'); title('Trajectory Plot with Terminal Set','FontSize',16);
hold on
set(gca,'FontWeight','bold')
plot(INVset_mpc_proj,'color','orange');
plot(realX(1,1),realX(2,1),'ogreen');
plot(realX(1,2:end),realX(2,2:end),'omagenta');
plot(x_cloop(1,2:end),x_cloop(2,2:end),'oblue');
legend('Feasible Set','C.A. Invariant Set','Initial States','Trajectory (Non-linear)','Trajectory (Linearized)','FontSize',12)
